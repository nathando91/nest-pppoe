...
async function killPppd(id) {
    ...
    // 1) Kill tracked PID first (instant, exact)
    var trackedPid = pppdPids.get(id);
    if (trackedPid) {
        await shellExec('kill -9 ' + trackedPid + ' 2>/dev/null');
        pppdPids.delete(id);
    }

    // 2) Find orphan pppd by exact cmdline match (safe: won't match nest_ppp10 when killing nest_ppp1)
    ...
    // 4) Wait for interface down + verify no survivors
    for (var w = 0; w < 10; w++) {
...
async function connectPppoe(id) {
    ...
    var child = spawn('pppd', ['call', peer], { detached: true, stdio: 'ignore' });
    ...
    // Failed — kill the pppd we just spawned (don't leave zombie)
