...
async function setupProxy(id, ip) {
    ...
    // Start 3proxy and track PID for 1-1-1 binding
    var proxyChild = spawn('3proxy', [cfgFile], { detached: true, stdio: 'ignore' });
    proxyPids.set(id, proxyChild.pid);
    proxyChild.unref();
    // Cleanup PID tracking if 3proxy exits
    proxyChild.on('exit', function() { if (proxyPids.get(id) === proxyChild.pid) proxyPids.delete(id); });
