...
async function killPppd(id) {
    ...
    // 2) Find orphan pppd by exact cmdline match
    ...
    if (allPids) {
        ...
        for (var p = 0; p < lines.length; p++) {
            ...
            if (cmd === exactCmd && pidsToKill.indexOf(pid) === -1) {
                pidsToKill.push(pid);
            }
        }
    }
...
async function connectPppoe(id) {
    ...
    // Spawn pppd with nopersist + maxfail 1:
    //   - nopersist: pppd exits immediately on failure (no internal retry loop)
    //   - maxfail 1: give up after 1 failed connection attempt
    //   This gives our code full control over retry logic
    var child = spawn('pppd', ['call', peer, 'nopersist', 'maxfail', '1'], { detached: true, stdio: 'ignore' });
...
async function scanExistingPids() {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        var pid = parseInt(parts[0]);
        var cmd = parts.slice(1).join(' ');
        // Extract session ID from "pppd call nest_pppN"
        var match = cmd.match(/^pppd call nest_ppp(\d+)$/);
        if (match) {
