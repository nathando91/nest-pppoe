...
const { connectPppoe, setupProxy, reconfigureProxy, spawnProxyWithGuard, rewriteProxyConfig, killProxy, killPppd, rebuildMacvlan, getSessionIP, getProxyPort, getProxyPorts, sleep, shellExec, isPrivateIP } = require('./pppoe');
...
function registerRoutes(app, io) {
    ...
    app.post('/api/config', function(req, res) {
        try {
            ...
            // (e.g. 'sweeper' toggle is managed separately and may not be in req.body)
            var existing = readConfig();
            var merged = Object.assign({}, existing, req.body);
            writeConfig(merged);
            // Re-sync remote hub connection with new config
            remote.refreshRemoteConfig();
    ...
    app.get('/api/sweeper', function(req, res) {
        var rotQ = require('./rotation');
        var config = readConfig();
        res.json({ enabled: rotQ.isSweeperEnabled(), config_value: config.sweeper !== false });
    });

    app.post('/api/sweeper', function(req, res) {
        try {
            var rotQ = require('./rotation');
            var enabled = !!req.body.enabled;
            rotQ.setSweeper(enabled);
    ...
    // ============ KILL PID for a specific session ============

    app.delete('/api/session/:id/pid', function(req, res) {
        ...
        (async function() {
            try {
                ...
                await killProxy(id);
                await killPppd(id);
                emitSessionState(io, id, { status: 'stopped', step: '', message: '' });
                io.emit('rotate_log', { id: id, data: '🔪 ppp' + id + ' PID killed by user\n' });
            } catch (e) { /* ignore */ }
        })();
    });
    ...
    app.get('/api/install-status', function(req, res) {
        try {
            ...
            // Peer files no longer needed — pppd args are injected dynamically at runtime
            peerCount = total;
            ...
            try {
                ...
                for (var m = 0; m < total; m++) {
                    var mInfo = getAccountForSession(config, m);
                    if (m === mInfo.offset) continue; // Skip physical interfaces
            ...
            var macvlanNeeded = 0;
            for (var mn = 0; mn < total; mn++) {
                var mnInfo = getAccountForSession(config, mn);
                if (mn !== mnInfo.offset) macvlanNeeded++;
            }
    ...
    app.post('/api/uninstall', function(req, res) {
        ...
        (async function() {
            try {
                ...
                // Remove legacy peer files (if any exist from old install)
                io.emit('install_log', '[1/4] Xoá peer files cũ (nếu có)...\n');
                await shellExec('rm -f /etc/ppp/peers/nest_ppp* 2>/dev/null');
                io.emit('install_log', '    ✅ Đã xoá peer files cũ\n');
    ...
    app.post('/api/install', function(req, res) {
        ...
        (async function() {
            try {
                ...
                io.emit('install_log', '[3/5] Tạo macvlan interfaces...\n');
                var createdMacvlan = 0;
                for (var i = 0; i < NUM; i++) {
                    var sInfo = getAccountForSession(config, i);
                    if (i === sInfo.offset) continue; // Skip physical interfaces

                    var macvlan = 'macppp' + i;
                    var macIface = sInfo.account.interface || DEFAULT_INTERFACE;
                ...
                // --- 4. PPPoE config (template mode) ---
                // pppd is now spawned with all args dynamically by Node.js
                // No peer files needed — credentials in chap-secrets, everything else at runtime
                io.emit('install_log', '[4/5] PPPoE config: template mode (Node.js inject)\n');
                // Clean up any legacy peer files from previous installs
                await shellExec('rm -f /etc/ppp/peers/nest_ppp* 2>/dev/null');
                io.emit('install_log', '    ✅ No peer files needed — args injected at runtime\n');

                // Setup static DNS (replaces usepeerdns to avoid race condition)
                try {
                    var resolv = fs.readFileSync('/etc/resolv.conf', 'utf8');
                    if (resolv.indexOf('8.8.8.8') === -1) {
                        fs.writeFileSync('/etc/resolv.conf', 'nameserver 8.8.8.8\nnameserver 8.8.4.4\n');
                        io.emit('install_log', '    ✅ DNS tĩnh: 8.8.8.8, 8.8.4.4\n');
                    } else {
                        io.emit('install_log', '    ✅ DNS đã cấu hình sẵn\n');
                    }
                } catch (e) {
                    fs.writeFileSync('/etc/resolv.conf', 'nameserver 8.8.8.8\nnameserver 8.8.4.4\n');
                    io.emit('install_log', '    ✅ DNS tĩnh: 8.8.8.8, 8.8.4.4\n');
    ...
    app.post('/api/kill-all-pid', function(req, res) {
        res.json({ success: true, message: 'Killing all PIDs...' });

        (async function() {
            try {
                await shellExec('pkill -9 pppd 2>/dev/null');
                await shellExec('pkill -9 3proxy 2>/dev/null');
                await shellExec('rm -f /var/run/ppp*.pid 2>/dev/null');
                await sleep(1000);

                var remaining = await shellExec('pgrep -c pppd 2>/dev/null || echo 0');
                var config = readConfig();
                var total = getTotalSessions(config);
                ...
                io.emit('stop_log', '[✓] All PIDs killed (remaining: ' + remaining.trim() + ')\\n');
                io.emit('stop_complete', { code: 0, output: 'All PIDs killed' });
                io.emit('refresh');
            } catch (err) {
    ...
    app.post('/api/stop-all', function(req, res) {
        ...
        (async function() {
            try {
                ...
                // Clear all proxies from NestProxy server
                io.emit('stop_log', '[*] Clearing proxies from server...\n');
                await nestproxy.clearAllProxies();
        ...
        (async function() {
            try {
                ...
                for (var n = 0; n < needStart.length; n++) {
                    ...
                    // Rebuild macvlan with new MAC (fresh start)
                    var nMac = await rebuildMacvlan(nId);
                    if (nMac) io.emit('start_log', '  🔧 ' + nIface + '  macvlan MAC: ' + nMac + '\n');
                    ...
                    if (nIp) {
                        ...
                        if (isPrivateIP(nIp)) {
                            ...
                            io.emit('start_log', '  ⚠️ ' + nIface + '  IP CGNAT: ' + nIp + ' — kill pppd, sweeper sẽ xử lý lại\n');
                            await killPppd(nId).catch(function() {});
                            continue;
                        }
                        ...
                        // Push proxy data to NestProxy server
                        await nestproxy.pushSessionProxies(nId, nIp, nResult.ports);
    ...
    app.post('/api/session/:id/start', function(req, res) {
        ...
        (async function() {
            var iface = 'ppp' + id;
            // Lock session to prevent sweeper from picking it concurrently
            var rotationQueue = require('./rotation');
            rotationQueue.lockSession(id);
            try {
                ...
                // Kill any existing pppd for this session first (prevent duplicates)
                await killPppd(id);

                // Rebuild macvlan with new MAC (fresh start)
                var mac = await rebuildMacvlan(id);
                if (mac) io.emit('rotate_log', { id: id, data: '   macvlan MAC: ' + mac + '\n' });
                ...
                if (isPrivateIP(ip)) {
                    ...
                    io.emit('rotate_log', { id: id, data: '⚠️ ' + iface + ' IP CGNAT: ' + ip + ' — kill pppd, sweeper sẽ xử lý lại\n' });
                ...
                // Unlock session so sweeper can manage it again
                rotationQueue.unlockSession(id);
            }
        })();
    });
    ...
    app.post('/api/session/:id/stop', function(req, res) {
        ...
        (async function() {
            // Lock session to prevent sweeper from picking it during shutdown
            var rotationQueue = require('./rotation');
            rotationQueue.lockSession(id);
            try {
                ...
                // Remove proxies from NestProxy server
                await nestproxy.removeSessionProxies(id);
    ...
    // ============ CHECK DIRECT via ppp interface (not proxy-dependent) ============

    function checkViaInterface(iface, timeoutMs) {
        ...
        return new Promise(function(resolve) {
            ...
            execFile('curl', [
                ...
                var latency = Date.now() - startTime;
                if (err) {
                    return resolve({ ok: false, error: 'curl: ' + (err.message || 'failed'), latency: latency });
                }
                var code = parseInt(stdout);
                resolve({ ok: code >= 200 && code < 400, latency: latency });
            });
        });
    }
    ...
    app.get('/api/session/:id/check', async function(req, res) {
        ...
        if (!ip) {
            return res.json({ id: id, ok: false, pppOk: false, proxyOk: false, error: 'no IP' });
        }

        // Check 1: direct via interface (source of truth)
        var ifaceResult = await checkViaInterface(iface);

        // Check 2: via 3proxy port
        var proxyOk = false;
        var proxyError = null;
        var proxyLatency = null;
        var autoFixedProxy = false;
        var portInfo = getProxyPorts(id);

        if (portInfo && portInfo.vipPort) {
            var proxyResult = await checkProxyTcp(parseInt(portInfo.vipPort));
            proxyOk = proxyResult.ok;
            proxyLatency = proxyResult.latency;
            proxyError = proxyResult.error || null;
        } else {
            proxyError = 'no proxy port';
        }

        // Auto-fix: PPPoE OK but proxy FAIL
        if (ifaceResult.ok && !proxyOk) {
            var cfgFile = require('path').join(__dirname, '..', 'proxy', '3proxy_ppp' + id + '_active.cfg');
            if (fs.existsSync(cfgFile)) {
                // Check if IP mismatch (pppd reconnected silently with new IP)
                var cfgContent = fs.readFileSync(cfgFile, 'utf8');
                var cfgIpMatch = cfgContent.match(/^# IP: (.+)$/m);
                var cfgIp = cfgIpMatch ? cfgIpMatch[1].trim() : null;

                if (cfgIp && cfgIp !== ip) {
                    // IP mismatch — reconfigure with correct IP + routing (keeps same ports)
                    var fixResult = await reconfigureProxy(id, ip);
                    autoFixedProxy = true;
                    proxyOk = true;
                    proxyError = null;
                    portInfo = { vipPort: String(fixResult.vipPort), ports: fixResult.ports.map(String) };
                    io.emit('rotate_log', { id: id, data: '🔧 [Check] ppp' + id + ': IP đổi (' + cfgIp + ' → ' + ip + ') — đã cấu hình lại proxy VIP:' + fixResult.vipPort + '\n' });
                } else {
                    // Same IP, just 3proxy died — restart with existing config
                    await killProxy(id);
                    await sleep(500);
                    ...
                    proxyOk = true;
                    proxyError = null;
                    io.emit('rotate_log', { id: id, data: '🔧 [Check] ppp' + id + ': 3proxy chết → đã restart\n' });
                }
            }
        }
        ...
        res.json({
            ...
            ip: ip,
            ok: ifaceResult.ok,
            pppOk: ifaceResult.ok,
            pppLatency: ifaceResult.latency,
            pppError: ifaceResult.error || null,
            proxyOk: proxyOk,
            proxyLatency: proxyLatency,
            proxyError: proxyError,
            port: portInfo ? portInfo.vipPort : null,
            autoFixedProxy: autoFixedProxy
        });
    });
    ...
    app.get('/api/check-all', async function(req, res) {
        ...
        // Run ALL checks in parallel — check via interface, not proxy
        var promises = [];
        for (var i = 0; i < totalSessions; i++) {
            (function(idx) {
                promises.push((async function() {
                    ...
                    var ip = await getSessionIP(iface);
                    if (!ip) {
                        var r = { id: idx, ok: false, pppOk: false, proxyOk: false, error: 'no IP' };
                    ...
                    // Auto-fix: PPPoE OK but proxy FAIL
                    if (ifaceResult.ok && !proxyOk) {
                        var cfgFile = require('path').join(__dirname, '..', 'proxy', '3proxy_ppp' + idx + '_active.cfg');
                        if (fs.existsSync(cfgFile)) {
                            var cfgContent = fs.readFileSync(cfgFile, 'utf8');
                            var cfgIpMatch = cfgContent.match(/^# IP: (.+)$/m);
                            var cfgIp = cfgIpMatch ? cfgIpMatch[1].trim() : null;
                            ...
                            if (cfgIp && cfgIp !== ip) {
                                var fixResult = await reconfigureProxy(idx, ip);
                                autoFixedProxy = true;
                                proxyOk = true;
                                portInfo = { vipPort: String(fixResult.vipPort), ports: fixResult.ports.map(String) };
                                io.emit('rotate_log', { id: idx, data: '🔧 [Check-All] ppp' + idx + ': IP đổi (' + cfgIp + ' → ' + ip + ') — đã cấu hình lại proxy\n' });
                            } else {
                                await killProxy(idx);
                                await sleep(300);
                                spawnProxyWithGuard(idx, cfgFile);
                                autoFixedProxy = true;
                                proxyOk = true;
                                io.emit('rotate_log', { id: idx, data: '🔧 [Check-All] ppp' + idx + ': 3proxy chết → đã restart\n' });
                            }
                        }
                    }
    ...
    app.post('/api/farm/:idx/stop', function(req, res) {
        ...
        (async function() {
            ...
            for (var i = range.start; i < range.end; i++) {
                ...
                await killProxy(i);
                await nestproxy.removeSessionProxies(i);
                await killPppd(i);
                emitSessionState(io, i, { status: 'stopped', step: 'done', message: '' });
            }
        ...
        (async function() {
            ...
            for (var n = 0; n < needStart.length; n++) {
                ...
                if (ip) {
                    ...
                    // Push proxy data to NestProxy server
                    await nestproxy.pushSessionProxies(nId, ip, result.ports);
                    connected++;
                } else {
                    emitSessionState(io, nId, { status: 'stopped', step: 'failed', message: 'Không nhận được IP' });
                }
        ...
        for (var i = range.start; i < range.end; i++) {
            (function(idx) {
                promises.push((async function() {
                    var iface = 'ppp' + idx;
                    var ip = await getSessionIP(iface);
                    if (!ip) return { id: idx, ok: false, pppOk: false, proxyOk: false, error: 'no IP' };

                    // Check via interface (source of truth)
                    var ifaceResult = await checkViaInterface(iface);
                    ...
                    if (ifaceResult.ok && !proxyOk) {
                        ...
                        if (fs.existsSync(cfgFile)) {
                            ...
                            if (cfgIp && cfgIp !== ip) {
                                ...
                                proxyOk = true;
                                portInfo = { vipPort: String(fixResult.vipPort), ports: fixResult.ports.map(String) };
                                io.emit('rotate_log', { id: idx, data: '🔧 [Farm-Check] ppp' + idx + ': IP đổi (' + cfgIp + ' → ' + ip + ') — đã cấu hình lại proxy\n' });
                            } else {
                                await killProxy(idx);
                                await sleep(300);
                                spawnProxyWithGuard(idx, cfgFile);
                                autoFixedProxy = true;
                                proxyOk = true;
                                io.emit('rotate_log', { id: idx, data: '🔧 [Farm-Check] ppp' + idx + ': 3proxy chết → đã restart\n' });
                            }
                        }
                    }
    ...
    // ============ NESTPROXY SERVER STATUS ============

    app.get('/api/server-status', function(req, res) {
        res.json(nestproxy.getSyncStatus());
    });

    app.post('/api/server-test', async function(req, res) {
        try {
            await nestproxy.pollProxyStatus();
            res.json({ success: true, status: nestproxy.getSyncStatus() });
        } catch (e) {
            res.status(500).json({ error: e.message });
        }
    });
