...
const { connectPppoe, setupProxy, spawnProxyWithGuard, rewriteProxyConfig, killProxy, killPppd, rebuildMacvlan, getSessionIP, getProxyPort, getProxyPorts, sleep, shellExec, isPrivateIP } = require('./pppoe');
...
function registerRoutes(app, io) {
    ...
    app.get('/api/session/:id/check', async function(req, res) {
        ...
        // Auto-fix: PPPoE OK but 3proxy dead → restart 3proxy
        if (ifaceResult.ok && !proxyOk) {
            var cfgFile = require('path').join(__dirname, '..', 'proxy', '3proxy_ppp' + id + '_active.cfg');
            var fs = require('fs');
            if (fs.existsSync(cfgFile)) {
                // Kill stale process first (cleanup), then respawn
                await killProxy(id);
                await sleep(500);
                spawnProxyWithGuard(id, cfgFile);
                autoFixedProxy = true;
                proxyOk = true;  // Optimistic — just restarted
                proxyError = null;
                io.emit('rotate_log', { id: id, data: '🔧 [Check] ppp' + id + ': PPPoE OK nhưng 3proxy chết → đã restart tự động\n' });
            }
        }
                ...
                promises.push((async function() {
                    ...
                    if (ifaceResult.ok && !proxyOk) {
                        ...
                        if (fs.existsSync(cfgFile)) {
                            ...
                            autoFixedProxy = true;
                            proxyOk = true;
                            io.emit('rotate_log', { id: idx, data: '🔧 [Check-All] ppp' + idx + ': PPPoE OK nhưng 3proxy chết → đã restart\n' });
                        }
                    }
                ...
                promises.push((async function() {
                    ...
                    if (ifaceResult.ok && !proxyOk) {
                        ...
                        if (fs.existsSync(cfgFile)) {
                            ...
                            autoFixedProxy = true;
                            proxyOk = true;
                            io.emit('rotate_log', { id: idx, data: '🔧 [Farm-Check] ppp' + idx + ': PPPoE OK nhưng 3proxy chết → đã restart\n' });
                        }
                    }
