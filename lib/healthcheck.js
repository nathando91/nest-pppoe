...
const { getSessionIP, getProxyPorts, isPrivateIP, spawnProxyWithGuard } = require('./pppoe');

const CHECK_INTERVAL_MS = 30 * 1000;   // Check every 30 seconds
const TCP_TIMEOUT_MS = 10000;           // 10s timeout per check
const BATCH_SIZE = 20;                  // Check 20 at a time to avoid overload
const MAX_FAILURES = 5;                 // Rotate after 5 consecutive failures
    ...
    for (var b = 0; b < sessionsToCheck.length; b += BATCH_SIZE) {
        ...
        for (var r = 0; r < results.length; r++) {
            ...
            if (res.ok) {
                ...
                // Increment fail count — only log, sweeper handles reconnect
                var count = (failCounts.get(res.id) || 0) + 1;
                failCounts.set(res.id, count);
                console.log('🏥 ppp' + res.id + ' connectivity FAILED (' + count + '): ' + (res.error || 'no response'));
            }
        }
    }
