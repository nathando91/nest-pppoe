...
const { getSessionIP, getProxyPorts, killPppd, killProxy, isPrivateIP } = require('./pppoe');
...
async function runHealthCheck() {
    ...
    var ipPromises = allSessionIds.map(async (i) => {
        if (inQueueIds[i] || stoppedSessions.has(i) || isInCooldown(i)) return null;
        var iface = 'ppp' + i;
    ...
    for (var i = 0; i < sessionInfos.length; i++) {
        ...
        if (isPrivateIP(s.ip)) {
            // CGNAT — kill pppd, sweeper will restart with new MAC
            console.log('🏥 ppp' + s.id + ' has CGNAT IP (' + s.ip + ') — killing pppd, sweeper will restart');
            if (io) io.emit('rotate_log', { id: s.id, data: '🏥 IP CGNAT ' + s.ip + ' (ISP cấp IP nội bộ) — đang ngắt, sweeper sẽ khởi động lại...\n' });
            setRotateCooldown(s.id, CGNAT_COOLDOWN_MS);
            killProxy(s.id).catch(function() {});
            killPppd(s.id).catch(function() {});
            continue;
        }
    ...
    for (var b = 0; b < sessionsToCheck.length; b += BATCH_SIZE) {
        ...
        for (var r = 0; r < results.length; r++) {
            ...
            if (res.ok) {
                ...
                // ---- 3proxy watchdog ----
                // Interface has internet — check if 3proxy is still alive
                // If it crashed silently, restart it using existing active config
                (function(sessionId) {
                    var cfgFile = require('path').join(__dirname, '..', 'proxy', '3proxy_ppp' + sessionId + '_active.cfg');
                    var fs = require('fs');
                    if (!fs.existsSync(cfgFile)) return; // No active config — session not fully set up

                    var { execFile } = require('child_process');
                    // Check if 3proxy is running for this session
                    execFile('pgrep', ['-f', '3proxy_ppp' + sessionId + '_active'], function(err) {
                        if (!err) return; // Still running — OK
                        // 3proxy is dead — restart it
                        console.log('🔧 3proxy ppp' + sessionId + ' died silently — restarting...');
                        if (io) io.emit('rotate_log', { id: sessionId, data: '🔧 3proxy ppp' + sessionId + ' bị crash, đang khởi động lại...\n' });
                ...
                if (count >= MAX_FAILURES) {
                    // Connectivity dead — kill proxy+pppd, sweeper will restart
                    console.log('🏥 ppp' + res.id + ' → killing (IP: ' + res.ip + ') — sweeper will restart');
                    if (io) {
                        io.emit('rotate_log', { id: res.id, data: '🏥 Mất mạng ' + count + ' lần liên tiếp, đang ngắt — sweeper sẽ xử lý lại...\n' });
                    }
                    failCounts.delete(res.id);
                    setRotateCooldown(res.id);
                    killProxy(res.id).catch(function() {});
                    killPppd(res.id).catch(function() {});
                }
            }
        }
    }
