...
async function runHealthCheck() {
    ...
    for (var i = 0; i < sessionInfos.length; i++) {
        ...
        if (isPrivateIP(s.ip)) {
            console.log('🏥 ppp' + s.id + ' has CGNAT IP (' + s.ip + '), auto-rotating...');
            if (io) {
                io.emit('rotate_log', { id: s.id, data: '🏥 IP CGNAT ' + s.ip + ' (ISP cấp IP nội bộ), tự động xoay lại...\n' });
            }
            setRotateCooldown(s.id, CGNAT_COOLDOWN_MS);
            rotationQueue.addRequest(s.id);
            continue;
        }
