...
const WATCHDOG_DELAY_MS = 3 * 1000;     // 3s delay between each proxy check in watchdog
const WATCHDOG_RESTART_COOLDOWN_MS = 60 * 1000; // 60s cooldown after restart before re-checking same session
...
// Track last watchdog restart time per session (for cooldown)
var lastWatchdogRestart = new Map();
var io = null;
...
function init(socketIo, rotQueue) {
    ...
    setTimeout(function() {
        ...
        runHealthCheck();
        // Start proxy watchdog — continuous round-robin liveness check
        console.log('🐕 Proxy watchdog started (delay: ' + (WATCHDOG_DELAY_MS / 1000) + 's per session)');
        runProxyWatchdog();
    }, 60 * 1000);
}
...
// Quick TCP liveness check through proxy (used by health check watchdog)
function checkProxyTcp(proxyPort, timeout) {
    ...
    return new Promise(function(resolve) {
        ...
        socket.on('data', function(data) {
            if (resolved) return;
            resolved = true;
            var ok = data.toString().indexOf('200') !== -1;
            socket.destroy();
            resolve(ok);
        });
        socket.on('timeout', function() {
...
async function runHealthCheck() {
    ...
    if (healthySessions.length > 0) {
        try {
            ...
            // ---- Proxy end-to-end connectivity check ----
            // Detect broken proxy (frozen, wrong IP binding, routing issues)
            var proxyChecks = [];
            for (var t = 0; t < healthySessions.length; t++) {
                var tid = healthySessions[t];
                if (!running3proxyIds.has(tid)) continue;
                var tPortInfo = getProxyPorts(tid);
                if (!tPortInfo || !tPortInfo.vipPort) continue;
                proxyChecks.push({ id: tid, port: parseInt(tPortInfo.vipPort) });
            }

            if (proxyChecks.length > 0) {
                var proxyResults = await Promise.all(proxyChecks.map(function(pc) {
                    return checkProxyConnectivity(pc.port).then(function(result) {
                        return { id: pc.id, ok: result.ok, error: result.error, latency: result.latency };
                    });
                }));

                for (var pr = 0; pr < proxyResults.length; pr++) {
                    if (!proxyResults[pr].ok) {
                        var fid = proxyResults[pr].id;
                        console.log('🏥 ppp' + fid + ' proxy NOT WORKING (end-to-end check failed: ' + (proxyResults[pr].error || 'no response') + ') — restarting...');
                        if (io) io.emit('rotate_log', { id: fid, data: '🔧 [HealthCheck] ppp' + fid + ': proxy không hoạt động (' + (proxyResults[pr].error || 'no response') + ') → restart\n' });
                        await killProxy(fid);
                        await new Promise(function(resolve) { setTimeout(resolve, 500); });
                        var fCfgFile = path.join(__dirname, '..', 'proxy', '3proxy_ppp' + fid + '_active.cfg');
                        if (fs.existsSync(fCfgFile)) {
                            spawnProxyWithGuard(fid, fCfgFile);
                            if (io) io.emit('rotate_log', { id: fid, data: '✅ [HealthCheck] ppp' + fid + ': proxy đã restart\n' });
                        }
                    }
                }
            }
...
// ---- Proxy Watchdog ----
// Continuously cycles through all sessions one-by-one, checking if 3proxy is alive.
// If frozen/dead → restart immediately. Runs independently from the main health check.
async function runProxyWatchdog() {
    while (true) {
        if (!running) {
            await new Promise(function(resolve) { setTimeout(resolve, WATCHDOG_DELAY_MS); });
            continue;
        }
        ...
        for (var i = 0; i < total; i++) {
            ...
            // Skip if recently restarted by watchdog
            var lastRestart = lastWatchdogRestart.get(i) || 0;
            if (Date.now() - lastRestart < WATCHDOG_RESTART_COOLDOWN_MS) continue;

            // Get proxy port info
            var portInfo = getProxyPorts(i);
            if (!portInfo || !portInfo.vipPort) continue;

            var port = parseInt(portInfo.vipPort);

            // Check if 3proxy config exists (session is set up)
            var cfgFile = path.join(__dirname, '..', 'proxy', '3proxy_ppp' + i + '_active.cfg');
            if (!fs.existsSync(cfgFile)) continue;

            // Quick TCP liveness check
            var alive = await checkProxyTcp(port, 5000);

            if (!alive) {
                // Double-check: is the ppp interface even up?
                var ip = await getSessionIP('ppp' + i);
                if (!ip) continue; // Interface down — main health check handles this

                console.log('🐕 [Watchdog] ppp' + i + ' proxy NOT responding on port ' + port + ' — restarting...');
                if (io) io.emit('rotate_log', { id: i, data: '🐕 [Watchdog] ppp' + i + ': proxy không phản hồi → restart\n' });

                await killProxy(i);
                await new Promise(function(resolve) { setTimeout(resolve, 500); });

                if (fs.existsSync(cfgFile)) {
                    spawnProxyWithGuard(i, cfgFile);
                    lastWatchdogRestart.set(i, Date.now());
                    if (io) io.emit('rotate_log', { id: i, data: '✅ [Watchdog] ppp' + i + ': proxy đã restart\n' });
                }
            }

            // Delay before checking next session
            await new Promise(function(resolve) { setTimeout(resolve, WATCHDOG_DELAY_MS); });
        }
