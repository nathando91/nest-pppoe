...
async function sweepFarm(farmIdx, state) {
    ...
    // Try to rotate stopped sessions with a concurrency limit of 2
    var activeCount = 0;
    ...
    for (var k = 0; k < stoppedIds.length; k++) {
        ...
        if (activeCount >= CONCURRENCY_LIMIT) {
            // If we reached limit, stop this sweep loop (it will run again in 8s)
            break;
        }
        ...
        // Start rotation without awaiting (async)
        activeCount++;
        tryRotateOnce(id, { manual: false }).finally(() => {
            // Note: we don't need to decrement activeCount here because 
            // the loop finishes and the next interval starts fresh.
        });
    }
}
...
async function tryRotateOnce(id, options) {
    ...
    try {
        ...
        if (!newIp) {
            ...
            emitSessionState(id, { ip: '', status: 'stopped', step: '', message: '' });
            if (io && options.manual) {
                io.emit('rotate_log', { id: id, data: '⚠️ ' + iface + ' CGNAT: ' + newIp + ' — bỏ qua\n' });
