...
async function executeRotation(id) {
    ...
    try {
        ...
        if (isDuplicate) {
            ...
            if (io) io.emit('rotate_log', { id: id, data: '   ⚠️ IP trùng (' + newIp + ' đã có trong IP.txt), thử lại (lần ' + entry.attempts + ')...\n' });
        ...
        // Wait for proxy to fully start before any checks
        await sleep(5000);

        // ---- POST-ROTATION CONNECTIVITY CHECK ----
        // Only run connectivity check for recovery scenarios (no old IP or same IP)
        // When IP successfully changed, skip check — health check will monitor later
        var skipConnCheck = entry.oldIp && newIp !== entry.oldIp;

        if (!skipConnCheck) {
            // Recovery scenario — verify connectivity
            ...
            if (!hasInternet) {
                ...
                // Schedule retry with reasonable delay (not too fast)
                entry.status = 'pending_retry';
                entry.newIp = '';
                entry.nextRetryAt = Date.now() + NO_INTERNET_RETRY_DELAY_MS;
                ...
                if (io) io.emit('rotate_log', { id: id, data: '   ⏳ Sẽ xoay lại sau ' + (NO_INTERNET_RETRY_DELAY_MS / 1000) + 's (lần ' + (entry.attempts + 1) + ')...\n' });
                emitQueueUpdate();
                return;
            }
        } else {
            if (io) io.emit('rotate_log', { id: id, data: '   ✅ IP đã đổi, bỏ qua connectivity check.\n' });
        }
