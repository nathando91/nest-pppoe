...
async function tryRotateOnce(id, options) {
    ...
    try {
        ...
        } else if (isIpUsed(newIp)) {
            // Duplicate IP (used within last 1h) — kill pppd and let sweeper retry
            await killPppd(id).catch(function() {});
            entry.status = 'failed';
            entry.message = '⚠️ IP trùng (' + newIp + ') — thử lại lần sau';
            emitQueueUpdate();
            emitSessionState(id, { ip: '', status: 'stopped', step: '', message: '' });
            if (io) {
                io.emit('rotate_log', { id: id, data: '⚠️ ' + iface + ' IP trùng: ' + newIp + ' — kill & thử lại lần sau\n' });
                io.emit('session_update', { id: id, output: '⚠️ IP trùng', code: 1 });
            }
