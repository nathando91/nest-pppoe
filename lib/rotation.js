...
var IP_EXPIRE_MS = 1 * 60 * 60 * 1000; // 1 hour
...
const SWEEP_INTERVAL_MS  = 5 * 1000;   // Time between sweeps (after a full pass)
...
const SESSION_COOLDOWN_MS = 30 * 1000; // Per-session cooldown after a failed rotation
...
// Per-session cooldown: id → timestamp of last failed attempt
var sessionCooldowns = new Map();
...
async function runFarmSweeper(farmIdx, state) {
    while (state.active) {
        ...
        try {
            ...
            console.error('[Rotation] Farm ' + farmIdx + ' sweeper error:', e.message);
        }
...
async function sweepFarm(farmIdx, state) {
    ...
    var logPrefix = '[Rotation] Farm ' + farmIdx;
    ...
    // Find all stopped sessions (no active IP, or CGNAT IP, or missing proxy) that are not already rotating
    var stoppedIds = [];
    var now = Date.now();
    for (var s = offset; s < offset + maxSess; s++) {
        if (rotatingSet.has(s)) continue;

        // SKIP if session was manually stopped by user (single session stop)
        // Note: Stop All is handled separately — sweeper will clear those marks
        if (healthCheck.isStopped && healthCheck.isStopped(s)) continue;

        // SKIP if session is in cooldown (recently failed)
        var lastFail = sessionCooldowns.get(s);
        if (lastFail && (now - lastFail) < SESSION_COOLDOWN_MS) continue;

        var ip = await getSessionIP('ppp' + s).catch(function() { return null; });
        
        // Check if proxy is running (source of truth)
        // If IP exists and is public, we expect a proxy process to be alive
        var proxyAlive = true;
        if (ip && !isPrivateIP(ip)) {
            proxyAlive = !!getPppoe().getProxyPid(s);
        }

        // Include sessions with no IP OR with a CGNAT/private IP OR missing proxy
        if (!ip || isPrivateIP(ip) || !proxyAlive) {
            stoppedIds.push(s);
        }
    }
    ...
    for (var k = 0; k < stoppedIds.length; k++) {
        ...
        if (recheck && !isPrivateIP(recheck)) {
            // Check if proxy is also alive (source of truth)
            if (!!getPppoe().getProxyPid(id)) continue; 
        }

        // ATOMIC LOCK: Add to rotatingSet BEFORE any further awaits or logic
        rotatingSet.add(id);
        pickedId = id;
        break;
    }
    ...
    if (pickedId !== -1) {
        console.log(logPrefix + ' Picking session ppp' + pickedId);
        await tryRotateOnce(pickedId, { manual: false, skipAdd: true });
    }
}
...
// Check internet connectivity via ppp interface before pushing to NestProxy
function checkConnectivity(iface) {
...
async function tryRotateOnce(id, options) {
    ...
    try {
        ...
        if (!newIp) {
            // No IP — skip, set cooldown
            await killPppd(id).catch(function() {});
            entry.status = 'failed';
            entry.message = '⚠️ Không có IP — chờ ' + (SESSION_COOLDOWN_MS / 1000) + 's';
            sessionCooldowns.set(id, Date.now());
            emitQueueUpdate();
            emitSessionState(id, { ip: '', status: 'stopped', step: '', message: '' });
            if (io) {
                io.emit('rotate_log', { id: id, data: '⚠️ ' + iface + ' không nhận được IP — chờ ' + (SESSION_COOLDOWN_MS / 1000) + 's trước khi thử lại\n' });
                if (options.manual) io.emit('session_update', { id: id, output: '⚠️ Không có IP', code: 1 });
            }

        } else if (isPrivateIP(newIp)) {
            // CGNAT — kill pppd and set cooldown before retry
            await killPppd(id).catch(function() {});
            entry.status = 'failed';
            entry.message = '⚠️ IP CGNAT (' + newIp + ') — chờ ' + (SESSION_COOLDOWN_MS / 1000) + 's';
            sessionCooldowns.set(id, Date.now());
            emitQueueUpdate();
            emitSessionState(id, { ip: '', status: 'stopped', step: '', message: '' });
            // Always log CGNAT (not just manual) so sweeper auto-rotations are visible
            if (io) {
                io.emit('rotate_log', { id: id, data: '⚠️ ' + iface + ' CGNAT: ' + newIp + ' — chờ ' + (SESSION_COOLDOWN_MS / 1000) + 's trước khi thử lại\n' });
                io.emit('session_update', { id: id, output: '⚠️ IP CGNAT', code: 1 });
            }

        // NOTE: Duplicate IP check disabled — always accept public IPs
        } else {
            ...
            // Check internet connectivity before pushing to NestProxy
            if (io) io.emit('rotate_log', { id: id, data: '   Kiểm tra kết nối trước khi đẩy lên server...\n' });
            var isOnline = await checkConnectivity(iface);
            if (!isOnline) {
                console.log('[Rotation] ppp' + id + ' — connectivity check FAILED before push, skipping NestProxy sync');
                if (io) io.emit('rotate_log', { id: id, data: '⚠️ ' + iface + ': IP OK nhưng mạng chưa thông — chờ ' + (SESSION_COOLDOWN_MS / 1000) + 's\n' });
                // Treat as failed so sweeper retries after cooldown
                await killProxy(id).catch(function() {});
                await killPppd(id).catch(function() {});
                entry.status = 'failed';
                entry.message = '⚠️ Mạng chưa thông — chờ ' + (SESSION_COOLDOWN_MS / 1000) + 's';
                sessionCooldowns.set(id, Date.now());
                emitQueueUpdate();
                emitSessionState(id, { ip: '', status: 'stopped', step: '', message: '' });
            } else {
                // SUCCESS — clear cooldown and mark as started
                sessionCooldowns.delete(id);
        ...
        await killPppd(id).catch(function() {});
        entry.status = 'failed';
        entry.message = '❌ Lỗi: ' + err.message + ' — chờ ' + (SESSION_COOLDOWN_MS / 1000) + 's';
        sessionCooldowns.set(id, Date.now());
        emitQueueUpdate();
        emitSessionState(id, { ip: '', status: 'stopped', step: 'error', message: '' });
        if (io) {
            io.emit('rotate_log', { id: id, data: '❌ ' + iface + ' lỗi: ' + err.message + ' — chờ ' + (SESSION_COOLDOWN_MS / 1000) + 's\n' });
            if (options.manual) io.emit('session_update', { id: id, output: 'ERROR: ' + err.message, code: 1 });
        }
...
/**
 * lockSession / unlockSession: external routes (single start/stop) use these
 * to prevent sweeper from picking the same session concurrently.
 */
