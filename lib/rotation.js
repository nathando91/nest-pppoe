...
async function sweepFarm(farmIdx, state, instanceId) {
    ...
    for (var s = offset; s < offset + maxSess; s++) {
        ...
        // SKIP if session was manually stopped by user
    ...
    try {
        ...
        // NOTE: Duplicate IP check disabled — always accept public IPs
            ...
            if (!isOnline) {
                ...
                if (io) io.emit('rotate_log', { id: id, data: '⚠️ ' + iface + ': IP OK nhưng mạng chưa thông — bỏ qua push NestProxy, sweeper sẽ thử lại...\n' });
                // Treat as failed so sweeper retries on next pass
                await killProxy(id).catch(function() {});
                await killPppd(id).catch(function() {});
                entry.status = 'failed';
                entry.message = '⚠️ Mạng chưa thông, thử lại lần sau';
                emitQueueUpdate();
                emitSessionState(id, { ip: '', status: 'stopped', step: '', message: '' });
            } else {
