...
const { getSessionIP, getProxyPorts, killProxy, killPppd, rebuildMacvlan, connectPppoe, setupProxy, sleep, isPrivateIP } = require('./pppoe');
...
const RETRY_DELAY_MS = 10 * 1000; // 10s between retries
...
const NO_INTERNET_RETRY_DELAY_MS = 10 * 1000; // 10s delay for no internet
const CGNAT_RETRY_DELAY_MS = 10 * 1000; // 10s delay for CGNAT IPs

// In-memory rotation queue
// Map<sessionId, { id, iface, oldIp, newIp, requestedAt, status, attempts, lastAttemptAt, nextRetryAt, message }>
var queue = new Map();
var io = null;
// Per-farm active count: Map<farmIdx, number>
var farmActiveCount = new Map();

function getFarmIdx(id) {
    var config = readConfig();
    var info = getAccountForSession(config, id);
    return info.accountIdx;
}

function getFarmActive(farmIdx) {
    return farmActiveCount.get(farmIdx) || 0;
}

function incrementFarm(farmIdx) {
    farmActiveCount.set(farmIdx, getFarmActive(farmIdx) + 1);
}

function decrementFarm(farmIdx) {
    farmActiveCount.set(farmIdx, Math.max(0, getFarmActive(farmIdx) - 1));
}
...
var MAX_CONCURRENT_PER_FARM = 5; // Max parallel rotations per farm

// Process queued items with per-farm concurrency limit
function processNext() {
    ...
    // Track how many extra slots we've filled per farm in this call
    var pendingByFarm = new Map();
    queued.forEach(function(entry) {
        var farmIdx = getFarmIdx(entry.id);
        var active = getFarmActive(farmIdx) + (pendingByFarm.get(farmIdx) || 0);
        if (active < MAX_CONCURRENT_PER_FARM) {
            pendingByFarm.set(farmIdx, (pendingByFarm.get(farmIdx) || 0) + 1);
            executeRotation(entry.id);
        }
    });
}
...
async function executeRotation(id) {
    ...
    var farmIdx = getFarmIdx(id);
    incrementFarm(farmIdx);
    ...
    try {
        ...
        // Clear proxies on NestProxy server before rotating
        await nestproxy.removeSessionProxies(id);
        await nestproxy.notifyVmChangingIp(id);
        ...
        // Rebuild macvlan with new MAC (ensures different IP from ISP)
        ...
        if (mac && io) io.emit('rotate_log', { id: id, data: '   macvlan MAC mới: ' + mac + '\n' });

        // Reconnect with new macvlan
        ...
        // If still no IP, retry reconnect once more
        if (!newIp) {
            if (io) io.emit('rotate_log', { id: id, data: '   ⚠️ Không nhận IP, kill + thử lại...\n' });
            await killPppd(id);
            await sleep(2000);
        ...
        if (!newIp) {
            // No IP - kill pppd and schedule retry (never give up)
            await killPppd(id);
            ...
            if (io) {
                io.emit('rotate_log', { id: id, data: '⚠️ ' + iface + ' không nhận được IP, sẽ thử lại sau 10s (lần ' + entry.attempts + ')\n' });
            }
            emitQueueUpdate();
            return;
        }
        ...
        // Check if IP is CGNAT/private (ISP assigns internal IP → no internet)
        if (isPrivateIP(newIp)) {
            if (io) io.emit('rotate_log', { id: id, data: '   ⚠️ IP CGNAT/Private: ' + newIp + ' (ISP cấp IP nội bộ, không có internet)\n' });
            if (io) io.emit('rotate_log', { id: id, data: '   ⏳ Sẽ thử lại sau 10s (đợi ISP cấp IP public)...\n' });

            // Don't setup proxy for CGNAT IPs — it will never work
            await killPppd(id);
            var cgnatMac = await rebuildMacvlan(id);
            if (cgnatMac && io) io.emit('rotate_log', { id: id, data: '   Tạo lại macvlan (MAC: ' + cgnatMac + ')\n' });

            entry.status = 'pending_retry';
            entry.newIp = '';
            entry.nextRetryAt = Date.now() + CGNAT_RETRY_DELAY_MS;
            entry.message = '⏳ IP CGNAT (' + newIp + '), chờ 10s (lần ' + entry.attempts + ')';
            emitSessionState(id, { ip: '', status: 'stopped', step: 'waiting', message: 'IP CGNAT — chờ ISP cấp IP public...', nextRetryAt: entry.nextRetryAt });
            emitQueueUpdate();
            return;
        }

        // Check if IP is duplicate (same as old OR already in IP.txt)
        var isDuplicate = isIpUsed(newIp) || (entry.oldIp && newIp === entry.oldIp);
        if (isDuplicate) {
            // IP duplicate - retry until we get a unique IP (IP.txt is re-read each time)
            var reason = (entry.oldIp && newIp === entry.oldIp) ? 'giống IP cũ' : 'đã có trong IP.txt';
            if (io) io.emit('rotate_log', { id: id, data: '   ⚠️ IP trùng (' + newIp + ' ' + reason + '), thử lại (lần ' + entry.attempts + ')...\n' });
            ...
            if (retryIp && !isIpUsed(retryIp) && retryIp !== entry.oldIp) {
                ...
                if (!retryIp) {
                    await killPppd(id);
                    var retryMac2 = await rebuildMacvlan(id);
                    retryIp = await connectPppoe(id);
                }
                if (retryIp) {
                    // Setup proxy with current IP to keep it alive
                    var tempResult = await setupProxy(id, retryIp);
                    // Push to server so it's tracked
                    await nestproxy.pushSessionProxies(id, retryIp, tempResult.ports);
        ...
        // POST-ROTATION: Skip connectivity check — health check monitor handles ongoing verification
        // Previously this could hang for minutes blocking the queue
        if (io) io.emit('rotate_log', { id: id, data: '   ✅ Proxy đã cấu hình, bỏ qua connectivity check (health check sẽ giám sát).\n' });
        ...
        await nestproxy.pushSessionProxies(id, newIp, result.ports);
        await nestproxy.notifyVmReady(id);
        if (entry.oldIp && newIp !== entry.oldIp) {
            await nestproxy.notifyIpChange(id, entry.oldIp, newIp);
        }
        ...
        if (entry.oldIp && newIp !== entry.oldIp) {
            ...
            // Same IP slipped through (safety net) — keep proxy alive but schedule retry
            // The whole point of rotation is to get a NEW IP
            if (io) {
                io.emit('rotate_log', { id: id, data: '⚠️ IP không đổi: ' + newIp + ' (VIP:' + result.vipPort + '), sẽ thử lại...\n' });
                io.emit('session_update', { id: id, output: 'RETRY ' + newIp + ':' + result.vipPort, code: 0 });
            }
            entry.status = 'pending_retry';
            entry.nextRetryAt = Date.now() + RETRY_DELAY_MS;
            entry.message = '⏳ IP không đổi, chờ thử lại (lần ' + entry.attempts + ')';
            emitSessionState(id, { ip: newIp, vipPort: String(result.vipPort), ports: result.ports.map(String), status: 'connected', proxyStatus: 'running', step: 'waiting', message: 'IP không đổi, chờ thử lại...', nextRetryAt: entry.nextRetryAt });
            emitQueueUpdate();
            return;
        }
        ...
        // Kill pppd on any error — don't leave zombies
        await killPppd(id).catch(function() {});
        // Schedule retry instead of marking as failed forever
        entry.status = 'pending_retry';
        entry.nextRetryAt = Date.now() + RETRY_DELAY_MS;
        entry.message = '⚠️ Lỗi: ' + err.message + ' — thử lại sau 10s (lần ' + entry.attempts + ')';
        emitSessionState(id, { status: 'stopped', step: 'error', message: 'Lỗi: ' + err.message + ' — chờ thử lại...', nextRetryAt: entry.nextRetryAt });
        if (io) {
            io.emit('rotate_log', { id: id, data: '❌ Lỗi: ' + err.message + ' — sẽ thử lại sau 10s\n' });
            io.emit('session_update', { id: id, output: 'ERROR: ' + err.message, code: 1 });
        }
        emitQueueUpdate();
    } finally {
        // Always decrement active count and try next — even on early return
        decrementFarm(farmIdx);
        processNext();
    }
}
...
async function scanQueue() {
    ...
    for (var i = 0; i < entries.length; i++) {
        ...
        if (entry.status === 'pending_retry' && entry.nextRetryAt && now >= entry.nextRetryAt) {
            // Re-queue it, processNext will handle concurrency
            entry.status = 'queued';
            entry.message = 'Đang chờ xoay lại...';
            emitQueueUpdate();
        }
    }
    // Trigger processing for any newly queued items
    processNext();
}
