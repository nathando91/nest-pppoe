...
const NO_INTERNET_RETRY_DELAY_MS = 30 * 1000; // 30s delay before retrying when no internet
const CGNAT_RETRY_DELAY_MS = 3 * 60 * 1000; // 3 min delay for CGNAT IPs (ISP issue, not us)
const MAX_CONCURRENT = 3; // Max simultaneous rotations to prevent system overload
...
// Process next queued item if capacity available
function processNext() {
    ...
    queue.forEach(function(entry) {
        if (!next && entry.status === 'queued') {
            next = entry;
        }
    });

    if (next) {
        executeRotation(next.id);
    }
}
...
async function executeRotation(id) {
    ...
    if (activeCount >= MAX_CONCURRENT) {
        // Stay queued, processNext will pick it up later
        return;
    }
    ...
    try {
        ...
        if (!newIp) {
            if (io) io.emit('rotate_log', { id: id, data: '   ⚠️ Không nhận IP, thử lại...\n' });
            await sleep(2000);
        ...
        if (isPrivateIP(newIp)) {
            ...
            if (io) io.emit('rotate_log', { id: id, data: '   ⏳ Sẽ thử lại sau 3 phút (đợi ISP cấp IP public)...\n' });
            ...
            entry.message = '⏳ IP CGNAT (' + newIp + '), chờ 3 phút (lần ' + entry.attempts + ')';
