...
function checkAllDone() {
    ...
    for (var d = 0; d < doneSessionIds.length; d++) {
        ...
        (async function() {
            ...
            // Clear server proxies IMMEDIATELY (cleanup on server)
            ...
            if (ip) {
                log('Session ' + sessionId + ' regular proxies DONE — closing (killing pppd)');
                try {
                ...
                log('  Session ppp' + sessionId + ' closed — Sweeper will pick up for rotation');
            } else {
                recentlyKilled.delete(sessionId);
                log('Session ' + sessionId + ' regular proxies DONE but already offline');
            }
        })();
    }
}
