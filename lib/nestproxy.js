...
const getProxyPorts = function(id) { return getPppoe().getProxyPorts(id); };
const isPrivateIP = function(ip) { return getPppoe().isPrivateIP(ip); };
const isRotating = function(id) { 
    try { return require('./rotation').isRotating(id); } catch (e) { return false; }
};
...
// Sessions recently killed by checkAllDone, to prevent immediate re-sync
var recentlyKilled = new Set();

// Track when each session was last pushed to the server (for grace period)
// Map<sessionId, timestamp>
var sessionPushTime = new Map();

// Track sessions that have had at least one port USED (confirming real customer usage)
// Set<sessionId>
var sessionEverUsed = new Set();

// Grace period: don't kill a session via checkAllDone within this time after push
var PUSH_GRACE_PERIOD_MS = 90 * 1000; // 90 seconds
...
async function loadAndValidateTracking() {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        var ip = await getSessionIP(iface).catch(function() { return null; });
...
async function pushSessionProxies(sessionId, ip, ports) {
    ...
    // Chặn push nếu session đang bị xoá (DONE) — tránh push lại proxy cũ
    if (recentlyKilled.has(sessionId)) {
        log('pushSessionProxies: BLOCKED — session ' + sessionId + ' đang bị xoá');
        return;
    }
    ...
    // Record push timestamp for grace period protection
    sessionPushTime.set(sessionId, Date.now());
    // Reset USED tracking for this fresh session
    sessionEverUsed.delete(sessionId);
...
async function syncSessionProxies(sessionId, ip, ports) {
    ...
    // Chặn sync nếu session đang bị xoá (DONE) — tránh push lại proxy cũ
    if (recentlyKilled.has(sessionId)) {
        log('syncSessionProxies: BLOCKED — session ' + sessionId + ' đang bị xoá');
        return;
    }
 ...
 */
async function removeSessionProxies(sessionId, reason) {
    if (!isEnabled()) return;

    // Skip if no local tracking for this session
    ...
    log('removeSessionProxies: session=' + sessionId + ' VM=' + vmId + (reason ? ' Reason=' + reason : ''));
    emitSyncStatus();
}
...
async function pollProxyStatus() {
    ...
    try {
        ...
        if (statusData && typeof statusData === 'object') {
            ...
            // Check each session: if ALL its regular proxyIds are DONE → close it (kill pppd)
            // Sweeper will then pick it up for rotation since it's offline.
            checkAllDone();
...
async function checkAndRepushMissing(statusData) {
    ...
    for (var i = 0; i < total; i++) {
        ...
        } else if (isRotating(i) || recentlyKilled.has(i)) {
            // Already rotating or recently killed — skip
            kept++;
        } else {
            // NOT tracked locally — check if it's healthy and needs sync
            var iface = 'ppp' + i;
            var ip = await getSessionIP(iface);
            if (ip && !isPrivateIP(ip)) { // Added !isPrivateIP(ip)
 ...
 * Also tracks which sessions have had at least one USED port.
 */
    ...
    sessionProxies.forEach(function(data, sessionId) {
        ...
        for (var i = 0; i < data.proxyIds.length; i++) {
            ...
            if (status) {
                ...
                // Track USED status — confirms real customer usage
                if (upperStatus === 'USED') {
                    sessionEverUsed.add(sessionId);
                }

                if (upperStatus === 'DONE' && port) {
...
function checkAllDone() {
    ...
    sessionProxies.forEach(function(data, sessionId) {
        ...
        // GUARD 1: Grace period — don't kill sessions pushed less than 90s ago
        var pushTime = sessionPushTime.get(sessionId) || 0;
        if (now - pushTime < PUSH_GRACE_PERIOD_MS) {
            return; // Too fresh — skip
        }

        // GUARD 2: Must have been USED at least once by a real customer
        // This prevents killing sessions that go WAITING → DONE without ever being used
        if (!sessionEverUsed.has(sessionId)) {
            return; // Never used — skip
        }

        var allDone = true;
        var regularDone = true;
        
        for (var i = 0; i < data.proxyIds.length; i++) {
            var pid = String(data.proxyIds[i]);
            var status = (serverStatusData.get(pid) || '').toUpperCase();
            
            if (status !== 'DONE') {
                allDone = false;
                if (i > 0) regularDone = false; // i=0 is VIP, i>0 are regular ports
            }
        }

        // Close if all regular ports are DONE OR all ports are DONE
        // BUT: if VIP (index 0) is being used, it's NOT done yet.
        var vipPid = String(data.proxyIds[0]);
        var vipStatus = (serverStatusData.get(vipPid) || '').toUpperCase();
        
        if ((regularDone || allDone) && vipStatus !== 'USED') {
            doneSessionIds.push(sessionId);
        }
    });

    // Process done sessions:
    // 1. Xoá proxy trên server NGAY LẬP TỨC → người dùng không vào được proxy cũ
    // 2. Kill cả cụm 3proxy + pppd local
    // 3. Sweeper tự nhặt session offline → tạo mới hoàn toàn
    for (var d = 0; d < doneSessionIds.length; d++) {
        var sessionId = doneSessionIds[d];
        var data = sessionProxies.get(sessionId);
        if (!data) continue;

        log('Session ppp' + sessionId + ' ALL proxies DONE (was USED: ✓, age: ' + Math.round((now - (sessionPushTime.get(sessionId) || 0)) / 1000) + 's) → rotating');

        // REMOVE from tracking IMMEDIATELY so next poll doesn't see it
        sessionProxies.delete(sessionId);
        saveTracking();
        clearBlockedPorts(sessionId);
        recentlyKilled.add(sessionId);
        sessionPushTime.delete(sessionId);
        sessionEverUsed.delete(sessionId);
        
        (async function(sid, vmId) {
            var machineId = getMachineId();
            var iface = 'ppp' + sid;

            // Bước 1: Xoá proxy trên server NGAY — chặn người dùng truy cập proxy cũ
            try {
                await clearVmProxies(vmId, machineId);
                log('Session ' + sid + ' ALL proxies DONE → đã xoá trên server (VM=' + vmId + ')');
            } catch (e) {
                log('Session ' + sid + ' clearVmProxies error: ' + e.message);
            }

            // Bước 2: Kill cả cụm local (3proxy 6 port + pppd) — xoá sạch 1 phát
            var ip = await getSessionIP(iface).catch(function() { return null; });
            if (ip) {
                try {
                    await killProxy(sid);
                    await killPppd(sid);
                    log('  Session ppp' + sid + ' đã kill sạch (pppd + 6proxy)');
                } catch (e) {
                ...
                // Notify UI
                if (io) {
                    io.emit('nestproxy_session_done', { sessionId: sid });
                }
            } else {
                log('Session ' + sid + ' ALL proxies DONE — đã offline sẵn');
            }

            // Wait a bit then remove from recentlyKilled
            setTimeout(function() {
                recentlyKilled.delete(sid);
            }, 15000);
        })(sessionId, data.vmId);
    }
}
