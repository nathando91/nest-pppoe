...
const { readConfig, writeConfig, getAccountForSession } = require('./config');
...
// Track which ports have already been blocked (avoid redundant 3proxy reloads)
// Set<"sessionId:port">
var blockedPorts = new Set();

/**
 * Block individual DONE ports in 3proxy.
 * When a specific proxyId is DONE, find its local port and remove it from 3proxy config.
 */
function blockDonePorts() {
    var { disableProxyPort } = require('./pppoe');
    ...
    sessionProxies.forEach(function(data, sessionId) {
        ...
        for (var i = 0; i < data.proxyIds.length; i++) {
            ...
            if (status && status.toUpperCase() === 'DONE' && port) {
                ...
                if (!blockedPorts.has(key)) {
                    blockedPorts.add(key);
                    var result = disableProxyPort(sessionId, port);
                    if (result) {
                        log('  🚫 Port ' + port + ' DONE → blocked in 3proxy (ppp' + sessionId + ')');
                    }
                }
            }
        }
...
// Clear blocked ports tracking for a session (when session rotates)
function clearBlockedPorts(sessionId) {
    var prefix = sessionId + ':';
    blockedPorts.forEach(function(key) {
        if (key.indexOf(prefix) === 0) {
            blockedPorts.delete(key);
        }
    });
}
...
function init(socketIo) {
    ...
    // Load persisted proxy tracking data
    loadTracking();
    ...
    // Start status polling (every 4s, matching C# ServerProxyData)
